...
export default function Account() {
  ...
  const handleDownloadCertificate = async (reg: EventRegistration) => {
    try {
      ...
      // Format Cloudinary URLs specifically for Image elements
      let imageUrl = templateUrl.trim();
      if (imageUrl.includes('cloudinary.com')) {
        if (imageUrl.toLowerCase().endsWith('.pdf')) {
            imageUrl = imageUrl.replace(/\.pdf$/i, '.png');
        } else if (imageUrl.includes('/raw/upload/')) {
      ...
      img.onload = async () => {
        try {
          ...
          canvas.width = img.width || 1200; // fallback width
          canvas.height = img.height || 849;
          ...
          const scale = canvas.width / 1200; // Base scale on 1200px width
          ...
          ctx.font = `italic ${Math.round(26 * scale)}px "Times New Roman", Georgia, serif`;
          ctx.fillText("This is to proudly certify that", canvas.width / 2, canvas.height * 0.40);

          // 1. Participant Name
          ctx.fillStyle = '#0f172a'; 
          ctx.font = `bold ${Math.round(55 * scale)}px "Times New Roman", Georgia, serif`;
          ctx.fillText(nameText, canvas.width / 2, canvas.height * 0.47);

          // Gold Underline
          ctx.strokeStyle = '#d4af37'; 
          ctx.lineWidth = Math.max(3 * scale, 2);
          const nameWidth = ctx.measureText(nameText).width;
          const lineHalfWidth = Math.max((nameWidth / 2) + (50 * scale), 200 * scale);
          ctx.beginPath();
          ctx.moveTo((canvas.width / 2) - lineHalfWidth, canvas.height * 0.50);
          ctx.lineTo((canvas.width / 2) + lineHalfWidth, canvas.height * 0.50);
          ctx.stroke();

          // 2. Official Wording Line 1
          ctx.fillStyle = '#4b5563';
          ctx.font = `italic ${Math.round(22 * scale)}px "Times New Roman", Georgia, serif`;
          ctx.fillText("for their valuable participation and dedicated contribution in the event", canvas.width / 2, canvas.height * 0.56);

          // 3. Event Title
          ctx.fillStyle = '#0f172a';
          ctx.font = `bold ${Math.round(38 * scale)}px "Inter", Arial, sans-serif`;
          ctx.fillText(eventText, canvas.width / 2, canvas.height * 0.63);

          // 4. Official Wording Line 2
          ctx.fillStyle = '#6b7280';
          ctx.font = `italic ${Math.round(16 * scale)}px "Times New Roman", Georgia, serif`;
          ctx.fillText("organized by SRISHREE VISION FOUNDATION, in recognition of their sincere effort towards community welfare.", canvas.width / 2, canvas.height * 0.69);

          // 5. Date
          ctx.fillStyle = '#000000';
          ctx.font = `bold ${Math.round(24 * scale)}px "Times New Roman", Georgia, serif`;
          ctx.fillText(eventDateStr, canvas.width * 0.30, canvas.height * 0.80);

          // 6. Authorized Signature
          ctx.font = `bold italic ${Math.round(28 * scale)}px "Times New Roman", Georgia, serif`;
          ctx.fillText("Lion Dr. R. Srinivas", canvas.width * 0.70, canvas.height * 0.79);
          
          ctx.font = `bold ${Math.round(18 * scale)}px "Inter", Arial, sans-serif`;
          ctx.fillText("Director", canvas.width * 0.70, canvas.height * 0.825);
          ...
          link.href = dataUrl;
          link.download = `Certificate_${nameText.replace(/\s+/g, '_')}_${eventText.replace(/\s+/g, '_')}.png`;
          ...
          alert("An error occurred while generating the certificate image.");
        } finally {
          setDownloadingCert(null);
        }
      };
      ...
      img.onerror = () => {
        // Fallback to corsproxy if first load fails
        if (!img.src.includes('corsproxy')) {
          img.src = `https://corsproxy.io/?${encodeURIComponent(imageUrl)}`;
        } else {
          console.error("Failed to load certificate template image from:", imageUrl);
          alert("Failed to load certificate template image. Please try again or contact support.");
          setDownloadingCert(null);
        }
      };
